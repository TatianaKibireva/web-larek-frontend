...
export class ModalPresenter {
  ...
  constructor(events: IEvents, private cardModel: CardModel<any>, previewTemplate: HTMLTemplateElement) {
    ...
    this.cardPreview.button.addEventListener('click', () => {
      ...
      if (card) {
        ...
        } else {events.emit('card:add', { card });}
      }
    }) 
  }
  ...
  private updateButtonState(cardId: string): void {
    ...
    this.cardPreview.buttonText = isInBasket ? 'Удалить из корзины' : 'Добавить в корзину';
  }
