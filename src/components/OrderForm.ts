...
export class OrderForm extends Component<IFormState> {
  ...
  private _updatePaymentButtons(selectedMethod: PaymentMethod) {
    this._onlineButton.classList.toggle('button_alt-active', selectedMethod === 'online');
    this._offlineButton.classList.toggle('button_alt-active', selectedMethod === 'offline');
  }
